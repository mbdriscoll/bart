...

{
	...
	return linop_create(N, ksp_dims, N, cim_dims,
			&PTR_PASS(data)->base, nufft_apply, nufft_apply_adjoint, nufft_apply_normal, NULL, nufft_free_data, "nufft");
}
...
const struct operator_s* nufft_precond_create(const struct linop_s* nufft_op)
{
	...
	return operator_create(pdata->N, pdata->cim_dims, pdata->N, pdata->cim_dims, &/*PTR_PASS*/(pdata)->base, nufft_precond_apply, nufft_precond_del, "nufft_precond");
}
